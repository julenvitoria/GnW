...
--ok-label Apply \
--cancel-label Exit \
--menu "Selecciona con las flechas la opcion deseada:" 13 120 15 \
   1 "CFW \"slim\" con los parametros para 16MB" \
   2 "Retro-Go para CFW \"slim\" con los parametros para 16MB" \
   3 "CFW \"slim\" + Retro-Go (solo un paso pero menos seguro): roms en /home/$usuario/game-and-watch-retro-go/roms/" \
   4 "CFW \"NO slim\" con los parametros para 16MB" \
   5 "Retro-Go para CFW \"NO slim\" con los parametros para 16MB" \
   6 "CFW \"NO slim\"+ Retro-Go (solo un paso pero menos seguro): roms en /home/$usuario/game-and-watch-retro-go/roms/" 2>"${INPUT}"
menuitem=$(<"${INPUT}")
case $menuitem in
  1)clear
    dialog --backtitle "G&W $consola - Utilidades de flasheo" \
