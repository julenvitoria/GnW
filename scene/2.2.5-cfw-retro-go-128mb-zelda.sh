...
dialog --backtitle "G&W $consola - Utilidades de flasheo ------------------ INFO: 2.2.5-cfw-retro-go-128mb-zelda.sh Usuario = $usuario   ////   Consola seleccionada = $consola ------------------" \
--title "G&W CFW + Retro-Go 128MB /// INFO: Usuario=$usuario --- Consola seleccionada=$consola --- Roms en /home/$usuario/game-and-watch-retro-go/roms/" \
--ok-label Apply \
--cancel-label Exit \
--menu "
...
Selecciona con las flechas la opcion deseada:" 0 0 0 \
   ...
   1 "CFW con los parametros para 128MB" \
   2 "Compilar Retro-Go con los parametros para 128MB" \
   3 "Flashear Retro-Go con los parametros para 128MB" \
   4 "Descarga y restauracion de saves-states con parametros 128MB"   2>"${INPUT}"
menuitem=$(<"${INPUT}")
case $menuitem in
  H)clear
  ...
  2)clear
    ...
    if [ $ans -eq 0 ]; then
        ...
        make clean
        make -j$proc COMPRESS=lzma EXTFLASH_SIZE_MB=124 EXTFLASH_OFFSET=4194304 INTFLASH_BANK=2 GNW_TARGET=$consola COVERFLOW=$caratula
        cd -
        echo " "
        echo " "
  ...
  3)clear
    ...
    if [ $ans -eq 0 ]; then
        ...
        read -n 1 -s -r -p ""
        cd /home/$usuario/gameandwatch/game-and-watch-retro-go
        make -j$proc COMPRESS=lzma EXTFLASH_SIZE_MB=124 EXTFLASH_OFFSET=4194304 INTFLASH_BANK=2 GNW_TARGET=$consola COVERFLOW=$caratula flash
        cd -
        echo " "
        echo " "
        read -n 1 -s -r -p "Proceso concluido. Presiona cualquier tecla para continuar."
