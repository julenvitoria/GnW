...
dialog --backtitle "G&W $consola - Utilidades de flasheo ------------------ INFO: 2.2.5-cfw-retro-go-128mb-mario.sh Usuario = $usuario   ////   Consola seleccionada = $consola ------------------" \
...
case $menuitem in
  ...
  2)clear
    ...
    if [ $ans -eq 0 ]; then
        ...
        make clean
        make -j$proc COMPRESS=lzma EXTFLASH_SIZE_MB=127 EXTFLASH_OFFSET=1048576 INTFLASH_BANK=2 GNW_TARGET=$consola COVERFLOW=$caratula
        cd -
        echo " "
        echo " "
  ...
  3)clear
    ...
    if [ $ans -eq 0 ]; then
        ...
        read -n 1 -s -r -p ""
        cd /home/$usuario/gameandwatch/game-and-watch-retro-go
        make -j$proc COMPRESS=lzma EXTFLASH_SIZE_MB=127 EXTFLASH_OFFSET=1048576 INTFLASH_BANK=2 GNW_TARGET=$consola COVERFLOW=$caratula flash
        cd -
        echo " "
        echo " "
        read -n 1 -s -r -p "Proceso concluido. Presiona cualquier tecla para continuar."
