...
case $menuitem in
  ...
  2)clear
    ...
    if [ $ans -eq 0 ]; then
        ...
        make clean
        make -j$proc COMPRESS=lzma EXTFLASH_SIZE_MB=252 EXTFLASH_OFFSET=4194304 INTFLASH_BANK=2 GNW_TARGET=$consola COVERFLOW=$caratula
        cd -
        echo " "
        echo " "
  ...
  3)clear
    ...
    if [ $ans -eq 0 ]; then
        ...
        read -n 1 -s -r -p ""
        cd /home/$usuario/gameandwatch/game-and-watch-retro-go
        make -j$proc COMPRESS=lzma EXTFLASH_SIZE_MB=252 EXTFLASH_OFFSET=4194304 INTFLASH_BANK=2 GNW_TARGET=$consola COVERFLOW=$caratula flash
        cd -
        echo " "
        echo " "
        read -n 1 -s -r -p "Proceso concluido. Presiona cualquier tecla para continuar."
