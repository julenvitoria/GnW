...
case $menuitem in
  1)clear
    ...
    clear;;
  2)clear
    sed -i 's/^proc=.*$/'proc=\""$(nproc)"\"'/g' ./scene/2.2-retro-go.sh
    sed -i 's/^proc=.*$/'proc=\""$(nproc)"\"'/g' ./scene/2.2.1-retro-go-1mb.sh
    sed -i 's/^proc=.*$/'proc=\""$(nproc)"\"'/g' ./scene/2.2.2-cfw-retro-go-1mb.sh
    sed -i 's/^proc=.*$/'proc=\""$(nproc)"\"'/g' ./scene/2.2.3-cfw-retro-go-16mb.sh
    sed -i 's/^proc=.*$/'proc=\""$(nproc)"\"'/g' ./scene/2.2.4-cfw-retro-go-64mb.sh
    ./scene/2.2-retro-go.sh
    ./2-menu-scene.sh
    clear;;
  3)clear
    dialog --backtitle "Game&Watch - Utilidades de flasheo" \
